...
printf("\nX = %d \t Y = %d \t Z[0]= %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d\t Z[4] =%d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
IX = IX + 4; /* IX se modifica. Observa que el apuntador se mueve4 posiciones y cae en una dirección afuera del arreglo. Esto ocasionará unerror. */ Y = *IX; /* Y se modifica, toma el valor (basura) de una celdaincorrecta. Es un error que no señala el compilador del lenguaje C. */
printf("\nX = %d \t Y = %d \t Z[0]= %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d\t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
